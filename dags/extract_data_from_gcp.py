...
def load_to_sql(file_path):
    ...
    engine = sqlalchemy.create_engine(f"postgresql+psycopg2://{conn.login}:{conn.password}@titanics-with-astro-pgdd_e460e4-postgres-1:{conn.port}/{conn.schema}")
